...
export default class InputBar extends React.Component {
    ...
    return (
      <div class="form-group mt-2">
        <div class="form-group">
          ...
          <div>{buttonTray("form-group btn-group d-sm-none")}</div>
        ...
        {Object.values(this.dialogs).map(dialog =>
          this.state.openDialog === dialog ? <div>{dialog.content}</div> : null
        )}
      </div>
    );
  }
  ...
  handleFormulaAddRequest = value =>
    this.props.onSetFormulaRequest(this.formulaField.current.value + value);
  handleHelpClick = () => this.setDialog(this.dialogs.help);
  handleRollRequest = value => this.props.onRollRequest(value);
